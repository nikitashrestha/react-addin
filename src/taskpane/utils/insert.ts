...
const htmlContent =
  '<div><h1 style ="font-family:verdena; color:red;">HTML Header</h1><p style="color:blue;">This is html paragraph wirtten in word</p></div>';
...
export const insertHTML = async (_e: React.MouseEvent, html: string = htmlContent) => {
  return Word.run(async context => {
    ...
    const nextSelection = context.document.getSelection().insertText("Inserted HTML successfully", "After");
    nextSelection.select("End");
  });
};
